...
3. `parcel build index.html` 명령을 실행하면 dist 폴더에 빌드된 파일들이 생성된다. 이때 dist 폴더 안에 있는 index.html을 직접 열어서 확인하면 제대로 실행이 되지 않는다. 이는 Parcel이 스크립트 경로를 절대 경로로 지정했기 때문인데, `parcel build index.html --public-url ./` 명령을 통해 경로를 직접 설정하면 이 문제를 해결할 수 있다. 
