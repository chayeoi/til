...
5. 속기법으로 `background` 속성을 지정할 때, 첫 번째 오는 값은 `background-color`이고 두 번째에 오는 값은 `background-image`이다. 무조건 image가 color보다 위에 놓이게 된다. 먼저 선언한 image가 상위에 놓인다는 개념과 혼동하지 않도록 주의한다.
...
7. [웹 폰트](http://greenalice.tistory.com/25)
