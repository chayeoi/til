...
1. `align-items`는 해당 요소 내의 모든 아이템을 정렬할 때, align-self는 자신만 정렬할 때 사용한다.
2. 요소 간에 정렬 순서를 변경할 때에는 마크업 순서로 배치를 바꾸는 것이 아니라 `flexbox`의 order 프로퍼티를 사용하여 정렬 순서를 변경한다. order 프로퍼티의 **default는 0**이다.
3. `flexbox`는 자신의 container보다 width가 클 때, **각 아이템의 width를 알아서 비율에 맞게 계산**한다.
4. `flexbox`의 width가 container보다 width를 넘었을 때 줄을 변경하고 싶다면 `flex-wrap: wrap`을 설정하면 된다.
5. `flex-wrap`의 **default는 no-wrap**이다.
6. `flex-flow`로 `flex-direction`과 `flex-wrap`을 한 번에 설정할 수 있다.
7. `align-items`는 단일 행, 열을 정렬할 때 사용하고 `align-content`는 다중 행, 열을 정렬할 때 사용한다.
...
1. `text-align: center`는 해당 요소가 block 요소일 때 사용 가능하다.
...
10. `position: absolute` 처리된 요소는 이미 뜬 상태이기 때문에 `float` 프로퍼티를 부여하더라도 의미가 없다.
...
2. 디자인은 모바일 퍼스트 규칙을 준수하기로 한다. 그 후 데스크탑으로 확장한다.

<br />

### 38. `input`의 `display` 프로퍼티 값

1. `input`은 기본적으로 inline-block 요소이다.

<br />

### 39. `img`의 `display` 프로퍼티 값
...
1. `line-height: 1.5`는 가독성을 고려했을 때 가장 좋은 수준이다.
2. `line-height: 1.5px`이 아님에 주의한다.

<br />

### 49. `:nth-child`와 `:nth-of-type`
